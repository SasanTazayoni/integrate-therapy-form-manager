...
describe("Dashboard - handleCheckProgress", () => {
  ...
  test("resets clientFormsStatus and sets new email when email changes", async () => {
    ...
    vi.mocked(useClientContext).mockReturnValue({
      email: "",
      setEmail: setEmailMock,
      clientFormsStatus: { exists: true, clientStatus: "active" as const, forms: {} as never },
      setClientFormsStatus: setClientFormsStatusMock,
      successMessage: "",
      setSuccessMessage: vi.fn(),
      error: "",
    ...
    vi.mocked(clientsApi.fetchClientStatus).mockResolvedValue({
      ok: true,
      data: { exists: true, clientStatus: "active" as const, formsCompleted: 2, forms: {} as never },
    });
  ...
  test("resets confirmedEmail and updates successMessage when checking a different email", async () => {
    ...
    vi.mocked(clientsApi.fetchClientStatus)
  ...
  test("clears success message before loading so spinner is visible on re-fetch", async () => {
    const mockEmail = "test@example.com";
    const mockClientStatus = { exists: true, clientStatus: "active" as const, inactive: false, formsCompleted: 2, forms: {} as never };

    let resolveSecondFetch: (value: { ok: true; data: typeof mockClientStatus }) => void;
    const secondFetchPromise = new Promise<{ ok: true; data: typeof mockClientStatus }>((resolve) => {
