...
beforeAll(() => {
  ...
  vi.mocked(clientsApi.fetchClientStatus).mockResolvedValue({ ok: true, data: {} as never });
});
...
describe("Dashboard - handleSendForm", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(ClientContextModule.useClientContext).mockReturnValue({
      email: "",
  ...
  test("button does nothing if confirmedEmail is missing", async () => {
    vi.mocked(ClientContextModule.useClientContext).mockReturnValue({
      email: "",
      clientFormsStatus: { exists: true, clientStatus: "active" as const, forms: { [FORM_TYPE]: {} as never } as never },
      setEmail: setEmailMock,
      setClientFormsStatus: setClientFormsStatusMock,
      successMessage: "",
      setSuccessMessage: vi.fn(),
  ...
  test("calls sendFormToken with normalized email and prevents double-send", async () => {
    ...
    vi.mocked(clientsApi.fetchClientStatus).mockResolvedValue({
      ok: true,
      data: mockClientStatus,
    });

    vi.mocked(formsApi.sendFormToken).mockResolvedValue({ ok: true, data: {} as never });
  ...
  test("sets error and resets activeToken if sendFormToken fails", async () => {
    ...
    const FORM_TYPE_LOCAL = FORM_TYPES[2];
    ...
    const mockClientStatus = {
      ...
      formsCompleted: 1,
      forms: {
        [FORM_TYPE_LOCAL]: { submitted: false, activeToken: false },
      } as never,
    };
    ...
    vi.mocked(formsApi.sendFormToken).mockResolvedValue({
      ok: false,
    ...
    const sendButton = getByTestId(`send-${FORM_TYPE_LOCAL}-button`);
    fireEvent.click(sendButton);
    ...
    await waitFor(() => {
      ...
      const setClientFormsStatusMocked = vi.mocked(
        vi.mocked(ClientContextModule.useClientContext)().setClientFormsStatus
      );
      const setClientFormsStatusCalls = setClientFormsStatusMocked.mock.calls;
      const lastCall =
        setClientFormsStatusCalls[setClientFormsStatusCalls.length - 1];
      const updatedStatus = lastCall[0] as { forms: Record<string, { activeToken: boolean }> };
      expect(updatedStatus.forms[FORM_TYPE_LOCAL].activeToken).toBe(false);
    });
  });

  test("shows fallback error and resets activeToken if sendFormToken fails without error message", async () => {
    const mockEmail = "test@example.com";
    const FORM_TYPE_LOCAL = "YSQ";
    ...
    const sendButton = getByTestId(`send-${FORM_TYPE_LOCAL}-button`);
    fireEvent.click(sendButton);
    await findByText(`Failed to send ${FORM_TYPE_LOCAL} form`);
