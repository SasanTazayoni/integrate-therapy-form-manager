...
describe("Dashboard - handleSendAllForms", () => {
  ...
  test("calls sendMultipleFormTokens with normalized email and prevents double-send", async () => {
    ...
    vi.mocked(formsApi.sendMultipleFormTokens).mockResolvedValue({ ok: true, data: {} as never });

    const setEmailMockLocal = vi.fn();
    const setClientFormsStatusMockLocal = vi.fn();
    vi.mocked(ClientContextModule.useClientContext).mockReturnValue({
      email: "",
      clientFormsStatus: null,
      setEmail: setEmailMockLocal,
      setClientFormsStatus: setClientFormsStatusMockLocal,
      successMessage: "",
  ...
  test("displays error if sendMultipleFormTokens fails", async () => {
    ...
    vi.mocked(formsApi.sendMultipleFormTokens).mockResolvedValue({
      ok: false,
  ...
  test("displays unexpected error if sendMultipleFormTokens throws", async () => {
    ...
    vi.mocked(formsApi.sendMultipleFormTokens).mockImplementation(() => {
  ...
  test("displays fallback error if sendMultipleFormTokens returns ok: false without message", async () => {
    ...
    vi.mocked(formsApi.sendMultipleFormTokens).mockResolvedValue({ ok: false, data: { error: "" } });
